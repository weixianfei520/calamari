...
urlpatterns = patterns(
    ...
    #add by futong
    url(r'^cluster/(?P<fsid>[a-zA-Z0-9-]+)/target$',
        calamari_rest.views.v2.TargetViewSet.as_view({'get': 'list','post': 'create'}),
        name='cluster-target-list'),
    url(r'^cluster/(?P<fsid>[a-zA-Z0-9-]+)/target/(?P<target_id>\d+)$',
        calamari_rest.views.v2.TargetViewSet.as_view({'get': 'retrieve',
                                                      'patch': 'update',
                                                      'delete': 'destroy'}),
        name='cluster-target-detail'),



    url(r'^cluster/(?P<fsid>[a-zA-Z0-9-]+)/pool/(?P<pool_id>\d+)/lun$',
        calamari_rest.views.v2.LunViewSet.as_view({'get': 'list', 'post': 'create'}),
        name='cluster-pool-lun-list'),
    url(r'^cluster/(?P<fsid>[a-zA-Z0-9-]+)/pool/(?P<pool_id>\d+)/lun/(?P<lun_id>\d+)$',
        calamari_rest.views.v2.LunViewSet.as_view({'get': 'retrieve',
                                                       'patch': 'update',
                                                   'delete': 'destroy'}),
        name='cluster-pool-lun-detail'),
