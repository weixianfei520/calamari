...
from calamari_rest.serializers.v2 import PoolSerializer, CrushRuleSetSerializer, CrushRuleSerializer, CrushNodeSerializer, CrushTypeSerializer,\
    ...
    CliSerializer, TargetSerializer, LunSerializer
...
from calamari_rest.sservice.iscsi.tgt_manager import tgt_mgr, pool_mgr
...
class TargetViewSet(RPCViewSet, RequestReturner):

    serializer_class = TargetSerializer

    def list(self, request, fsid):
        tmg = tgt_mgr()
        targets = tmg.get_target_iqn_list()
        return Response(TargetSerializer(targets['target_list'], many=True).data)
    ...
    def create(self, request, fsid):
        ...
        if serializer.is_valid(request.method):
            ...
            iqn = data['iqn']
            try:
                tmg.create_target(iqn)
            except Exception, e:
                return Response(str(e), status=status.HTTP_409_CONFLICT)
            return Response('create targte(iqn:%s) ok' % data['iqn'], status=status.HTTP_202_ACCEPTED)
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def destroy(self, request, fsid, target_id):
        tmg = tgt_mgr()
        try:
            tmg.remove_target_by_id(int(target_id))
        except Exception, e:
            return Response(str(e), status=status.HTTP_404_NOT_FOUND)
        return Response(status=status.HTTP_202_ACCEPTED)

    def retrieve(self, request, fsid, target_id):
        tmg = tgt_mgr()
        try:
            target = tmg.get_target_by_id(int(target_id))
        except:
            return Response(status=status.HTTP_404_NOT_FOUND)
        return Response(TargetSerializer(target.conf).data)

    def update(self, request, fsid, target_id):
        serializer = self.serializer_class(data=request.DATA)
        tmg = tgt_mgr()
        if serializer.is_valid(request.method):
            try:
                iqn = serializer.get_data()
                target = tmg.get_target_by_id(int(target_id))
                target.update_target_iqn(iqn['iqn'])
            except Exception, e:
                return Response(str(e), status=status.HTTP_404_NOT_FOUND)
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class LunViewSet(RPCViewSet, RequestReturner):

    serializer_class = LunSerializer

    def list(self, request, fsid, pool_id):
        pool = PoolDataObject(self.client.get(fsid, POOL, int(pool_id)))
        pmg = pool_mgr(PoolSerializer(pool).data['name'])
        try:
            # create_response = self.client.create(fsid, 'RBD', {'pool': 'liupeng','image': 'hhh', 'size': 1*1024**3})
            # print create_response
            lun_obj = pmg.get_lun_by_pool()
        except Exception, e:
            print e
            return Response(str(e), status=status.HTTP_404_NOT_FOUND)
        return Response(LunSerializer(lun_obj, many=True).data)

    def create(self, request, fsid, pool_id):
        serializer = self.serializer_class(data=request.DATA)
        if serializer.is_valid(request.method):
            pool = PoolDataObject(self.client.get(fsid, POOL, int(pool_id)))
            pool_name = PoolSerializer(pool).data['name']
            data = serializer.get_data()
            tmg = tgt_mgr()
            try:
                target = tmg.get_target_by_iqn(data['iqn'])
                target.add_lun(pool_name, data['image'], data['size'], data['name'])
            except Exception, e:
                return Response(str(e), status=status.HTTP_404_NOT_FOUND)
            return Response(status=status.HTTP_202_ACCEPTED)
        else:
    ...
    def retrieve(self, request, fsid, pool_id, lun_id):
        pool = PoolDataObject(self.client.get(fsid, POOL, int(pool_id)))
        pmg = pool_mgr(PoolSerializer(pool).data['name'])
        try:
            lun_obj = pmg.get_lun_by_pool_and_num(int(lun_id))
        except Exception, e:
            return Response(str(e), status=status.HTTP_404_NOT_FOUND)
        return Response(LunSerializer(lun_obj).data)

    def update(self, request, fsid, pool_id, lun_id):
        serializer = self.serializer_class(data=request.DATA)
        pool = PoolDataObject(self.client.get(fsid, POOL, int(pool_id)))
        pmg = pool_mgr(PoolSerializer(pool).data['name'])
        if serializer.is_valid(request.method):
            ...
            try:
                lun_obj = pmg.get_lun_by_pool_and_num(int(lun_id))
                target = tmg.get_target_by_iqn(lun_obj['iqn'])
                target.set_lun(int(lun_id), data)
            except Exception, e:
                return Response(str(e), status=status.HTTP_404_NOT_FOUND)
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def destroy(self, request, fsid, pool_id, lun_id):
        pool = PoolDataObject(self.client.get(fsid, POOL, int(pool_id)))
        pmg = pool_mgr(PoolSerializer(pool).data['name'])
        try:
            lun_obj = pmg.get_lun_by_pool_and_num(int(lun_id))
            iqn = LunSerializer(lun_obj).data['iqn']
            target = pmg.get_target_by_iqn(iqn)
            target.del_lun(int(lun_id))
        except Exception, e:
            return Response(str(e), status=status.HTTP_404_NOT_FOUND)
        return Response(status=status.HTTP_202_ACCEPTED)
...
# class CloneViewSet(RPCViewSet, RequestReturner):
#
...
#         pass
#
#     def destroy(self,request,fsid,clone_id):
#         pass
#
#     def update(self,request,fsid,clone_id):
#         pass
